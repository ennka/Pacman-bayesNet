...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for i in range(len(livingGhostPositionDistributions)):
            positions=livingGhostPositionDistributions[i]
            maxBelief.append(max(positions,key=lambda x: positions[x]))
            #Find the most likely position that the ghost might in

        minValue = 99999999999
        result = None

        for action in legal:
            temp = min(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), ghost) for ghost in maxBelief)
            if minValue> temp :
                result = action
                minValue = temp
        return result
